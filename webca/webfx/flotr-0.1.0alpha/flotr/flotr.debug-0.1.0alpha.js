...
var Flotr = (function(){
	...
	function Plot(container, data, opts) {
		...
		function refresh(data, opts) {
			...
			ctx.clearRect(0, 0, target.getWidth(), target.getHeight());
			
            if (selectionInterval == null) {
            	octx.clearRect(0, 0, target.getWidth(), target.getHeight());
            }			
		...
		function setOptions(o){
			...
			options = merge(o, {
				...
				xaxis: {
					...
					tickFormatter: defaultTickFormatter, // => fn: number, par -> string
					tickFormatterPar: null, // => parameter to pass to tickFormatter after the number
				...
				mouse: {
					...
					trackDecimals: null,	// => decimals for the track values, default is calculated by pixel precision
		...
		function extendYRangeIfNeededByBar(){
			...
			/**
			 * If a bar is present, include 0 in displayed range.
			 */
			if (bar) {
				yaxis.datamin = Math.min(yaxis.datamin, 0);
				yaxis.datamax = Math.max(yaxis.datamax, 0);
			}
		}
		/**
		...
		function calculateTicks(axis, axisOptions){
			...
			if(axisOptions.ticks){
				...
				for(var i = 0, v, label; i < ticks.length; ++i){
					...
					if(typeof(t) == 'object'){
						v = t[0];
						label = (t.length > 1) ? t[1] : axisOptions.tickFormatter(v, axisOptions.tickFormatterPar);
					}else{
						v = t;
						label = axisOptions.tickFormatter(v, axisOptions.tickFormatterPar);
					}
				...
				for(i = 0; start + i * axis.tickSize <= axis.max; ++i){
					...
					axis.ticks.push({ v: v, label: axisOptions.tickFormatter(v, axisOptions.tickFormatterPar) });
				}
			}
		}
		/**
		...
		function calculateSpacing(){
			...
			plotOffset.bottom += 2 * labelMaxHeight + 2 * options.grid.labelMargin;			
		...
		function drawBaseLine(){
			if (options.bars.show) {
				...
				ctx.lineJoin = 'round';
	   			...
	   			ctx.beginPath();
				ctx.moveTo(0, Math.floor(tVert(0)) + ctx.lineWidth/2);
				ctx.lineTo(plotWidth, Math.floor(tVert(0)) + ctx.lineWidth/2);
	    		ctx.stroke();
		...
		function drawAxisLabels() {
			...
			var yPos = plotOffset.top + plotHeight + 2 * options.grid.labelMargin + labelMaxHeight;   
			
			var html = '<div style="font-size:smaller;color:' + options.grid.color + '">';
			if (options.xaxis.label != null) {
				html += '<div style="position:absolute;top:' + yPos + 'px;left:' + (plotOffset.left + plotWidth / 2) + 'px;text-align:center" class="flotr-grid-label">' + options.xaxis.label + "</div>";
			}
			if (options.yaxis.label != null) {
				html += '<div style="position:absolute;top:' + yPos + 'px;left:' + (options.grid.labelMargin) + 'px;text-align:left" class="flotr-grid-label">' + options.yaxis.label + "</div>";
			}
			html += '</div>';		
			target.insert(html);
		}		
		...
		function drawSeriesBars(series) {
			function plotBars(data, barWidth, offset, fill){
				...
				for(var i = 0; i < data.length; i++){
					var x = data[i][0], y = data[i][1];
					var drawLeft = true, drawTop = true, drawRight = true, drawBottom = true;
					var left = x, right = x + barWidth, bottom = (y >= 0) ? 0 : y, top = (y >= 0) ? y : 0;
					...
					if(drawLeft || drawRight || drawTop || drawBottom){
						...
						if(drawBottom) ctx.lineTo(tHoz(left), tVert(bottom) + offset);
						ctx.stroke();
					}
				}
			}
		...
		function fireSelectedEvent(){
			var selX1 = (selection.first.x <= selection.second.x) ? selection.first.x : selection.second.x;
			var selX2 = (selection.first.x <= selection.second.x) ? selection.second.x : selection.first.x;
			var selY1 = (selection.first.y >= selection.second.y) ? selection.first.y : selection.second.y;
			var selY2 = (selection.first.y >= selection.second.y) ? selection.second.y : selection.first.y;
			
			var x1 = xaxis.min + selX1 / hozScale;
			var x2 = xaxis.min + selX2 / hozScale;
			var y1 = yaxis.max - selY1 / vertScale;
			var y2 = yaxis.max - selY2 / vertScale;

			target.fire('flotr:select', [ { x1: x1, y1: y1, x2: x2, y2: y2 }, { x1: selX1, y1: selY1, x2: selX2, y2: selY2 } ]);
		}
		/**
		...
		function hit(mouse){
			...
			if(n.mouse && n.mouse.track && !prevHit || (prevHit && n.x != prevHit.x && n.y != prevHit.y)){
				...
				if(n.x !== null && n.y !== null){
					...
					if (decimals == null) {
						var pixPrec = Math.min((xaxis.max - xaxis.min) / plotWidth, (yaxis.max - yaxis.min) / plotHeight);
						decimals = -Math.floor(Math.log(pixPrec) / Math.LN10);
					}
					if (decimals < 0) decimals = 0;
